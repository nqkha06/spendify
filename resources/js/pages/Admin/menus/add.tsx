...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Create', href: adminMenus.create().url },
];
...
export default function CreateMenu({ parentMenus }: Props) {
    ...
    const availableParents = useMemo(() => {
        return parentMenus.filter((menu) => menu.canonical === data.canonical);
    }, [data.canonical, parentMenus]);

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(adminMenus.store().url);
    };
        ...
        <AdminLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 sm:gap-6">
                <div>
                    ...
                    <p className="text-muted-foreground">Add a menu item for a canonical slot with optional sub menu support.</p>
                </div>
